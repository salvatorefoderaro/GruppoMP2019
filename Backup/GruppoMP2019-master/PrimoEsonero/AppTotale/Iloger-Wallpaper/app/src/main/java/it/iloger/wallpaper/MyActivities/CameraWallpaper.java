...
public class CameraWallpaper extends AppCompatActivity {
        ...
        getImageFromGallery.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ContextCompat.checkSelfPermission(CameraWallpaper.this.getApplicationContext(), android.Manifest.permission.READ_EXTERNAL_STORAGE)== PackageManager.PERMISSION_GRANTED)
                {
                    ...
                    intent.setType(getString(R.string.intentTypeImage));
                    intent.setAction(Intent.ACTION_GET_CONTENT);
                    startActivityForResult(Intent.createChooser(intent, getString(R.string.intentTitle)), PICK_IMAGE_REQUEST);
                }
                else
                {
    ...
    private void captureCameraImage() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            File f = new File(Environment.getExternalStorageDirectory(), getString(R.string.tempFile));
        ...
        if (grantResults.length > 0) {
            ...
            if (requestCode == MY_READ_PERMISSION_REQUEST_CODE){
                ...
                intent.setAction(Intent.ACTION_GET_CONTENT);
                    ...
                    Toast.makeText(
                            getApplicationContext(),
                            getText(R.string.noStoragePermission).toString(),
                            Toast.LENGTH_LONG).show();
                }
            }
            if (requestCode == CAMERA_REQUEST_CODE) {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(
                            getApplicationContext(),
                            getText(R.string.noCameraPermission).toString(),
                            Toast.LENGTH_LONG).show();
                }
            }
        }
